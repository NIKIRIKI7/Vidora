...
public interface IYouTubeClient
{
    ...
    Task<IReadOnlyList<YouTubeVideoMetadata>> SearchFilteredAsync(string query, YouTubeSearchFilter? filter = null, int maxResults = 20, string lang = "ru", CancellationToken cancellationToken = default);
    ...
    Task<IReadOnlyList<YouTubeCommentItem>> GetCommentsDetailedAsync(string videoUrlOrId, int maxComments = 50, CancellationToken cancellationToken = default);
    ...
    Task<IReadOnlyList<YouTubeVideoChapter>> GetChaptersAsync(string videoUrlOrId, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<YouTubeHeatmapPoint>> GetHeatmapAsync(string videoUrlOrId, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<YouTubeWordTimestamp>> GetWordTimestampsAsync(string videoUrlOrId, CancellationToken cancellationToken = default);
    Task<YouTubeChannelStats> GetChannelStatsAsync(string channelId, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<YouTubeChannelUpload>> GetChannelRecentUploadsAsync(string channelId, int maxUploads = 15, string lang = "ru", CancellationToken cancellationToken = default);
