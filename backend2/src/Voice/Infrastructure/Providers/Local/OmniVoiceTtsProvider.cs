...
public sealed class OmniVoiceTtsProvider : ITtsEngineProvider
{
    ...
    {
        var modelTarget = "ai-models/omnivoice";
        ...
        var resolvedModelDir = ModelPathResolver.Locate(modelTarget, "data_storage");
        if (string.IsNullOrEmpty(resolvedModelDir))
        {
            throw new DomainConflictException("Локальная модель OmniVoice не найдена на диске.", "OMNIVOICE_NOT_FOUND");
        }
        ...
        var result = await _mlHost.ExecuteScriptAsync(
            scriptRelativePath: Path.Combine("tools", "scripts", "omnivoice_tts.py"),
            jsonPayload: payload,
