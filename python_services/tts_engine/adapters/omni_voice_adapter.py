...
VALID_INSTRUCTS = {
    ...
    "russian accent", "teenager", "very high pitch", "very low pitch", "young adult",
...
def _load_voice_clone_prompt(embedding_path: str):
    """Загрузка voice clone prompt (.pt), совместимая с torch>=2.6.

    В torch 2.6 у ``torch.load`` по умолчанию ``weights_only=True``, из-за чего
    старые ``.pt`` (сохранённые как объект ``VoiceClonePrompt``, а не dict)
    падают с ``_pickle.UnpicklingError``. Порядок: штатный ``load`` →
    ``safe_globals([VoiceClonePrompt])`` → фоллбэк ``weights_only=False``.
    """
    from omnivoice import VoiceClonePrompt

    try:
        return VoiceClonePrompt.load(embedding_path)
    except Exception as exc:  # noqa: BLE001
        logger.warning(
            "[VoiceClonePrompt] Штатная загрузка '%s' не удалась (%s). "
            "Пробую совместимые режимы загрузки.",
            embedding_path, exc,
        )

    data = None
    # torch>=2.6: allowlist класса достаточно для безопасной загрузки старых
    # чекпоинтов, где сохранён сам VoiceClonePrompt, а не dict.
    safe_globals = getattr(getattr(torch, "serialization", None), "safe_globals", None)
    if safe_globals is not None:
        try:
            with safe_globals([VoiceClonePrompt]):
                data = torch.load(embedding_path, map_location="cpu", weights_only=True)
        except Exception as exc:  # noqa: BLE001
            logger.debug("[VoiceClonePrompt] safe_globals-загрузка не удалась: %s", exc)

    if data is None:
        # Локальный доверенный файл: последний рубеж — отключаем weights_only.
        try:
            data = torch.load(embedding_path, map_location="cpu", weights_only=False)
        except TypeError:
            # torch < 2.6 не знает аргумент weights_only.
            data = torch.load(embedding_path, map_location="cpu")

    if isinstance(data, VoiceClonePrompt):
        return data
    if isinstance(data, dict):
        try:
            ...
            raise ValueError(
                f"Некорректный voice clone prompt '{embedding_path}': нет поля {exc}."
            ) from exc
    raise TypeError(
        f"Неподдерживаемый формат voice clone prompt '{embedding_path}': {type(data)!r}."
    )
...
class OmniVoiceAdapter(BaseVoiceEngine):
    ...
    def clone_voice(
        ...
        if not ref_text:
            raise ValueError(
                "Отсутствует reference_text для клонирования. Транскрипцию эталона "
                "должен выполнять C# (FasterWhisper.NET) и передавать её в поле "
                "reference_text. ASR внутри Python не используется."
            )
    ...
    def synthesize(
        ...
        if instruct:
            ...
            prompt = _load_voice_clone_prompt(embedding_path)
