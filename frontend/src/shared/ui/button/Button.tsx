...
export const Button = ({ variant = 'primary', icon: IconComp, filledIcon, children, className = '', ...props }: ButtonProps) => {
  ...
  const variants = {
    ...
    glass:
      'bg-primary/10 border border-primary/20 text-primary hover:bg-primary/20 shadow-[0_0_15px_rgba(221,183,255,0.15)] rounded-xl flex-col gap-1 w-16 h-16 text-[10px] font-medium leading-none shrink-0',
    icon: 'hover:bg-primary/10 text-on-surface-variant hover:text-primary rounded-full w-10 h-10 shrink-0',
    dashed:
