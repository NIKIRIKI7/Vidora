  ...
  return (
    <div className={`relative flex flex-col bg-surface border border-outline-variant rounded-xl p-4 backdrop-blur-xl ${className}`}>
      <div className="flex items-center justify-between mb-3 text-xs">
        ...
        <div className="text-on-surface-variant/70 font-mono text-2xs">
          {activeHover ? (
      ...
      <div className="relative w-full h-[var(--layout-chart)] overflow-hidden rounded-lg bg-surface-container-lowest border border-outline-variant/30">
        <div
        ...
        >
          <span className="absolute top-1 left-1.5 text-3xs font-mono text-primary uppercase tracking-wider bg-surface-container-lowest/50 px-1 rounded">
            Хук 0-15s
          </span>
        </div>
        ...
        <svg viewBox="0 0 1000 180" className="w-full h-full preserve-3d cursor-crosshair z-10" preserveAspectRatio="none">
          <defs>
            ...
            <linearGradient id="lineGradient" x1="0%" y1="0%" x2="100%" y2="0%">
              ...
              <stop offset="60%" style={{ stopColor: 'var(--color-secondary)' }} />
              <stop offset="100%" style={{ stopColor: 'var(--color-primary-container)' }} />
            </linearGradient>
          </defs>
            ...
            return (
              <g key={idx}>
                <rect
                  ...
                  width={w}
                  height={180}
                  style={{ fill: isHovered ? 'color-mix(in srgb, var(--color-primary) 15%, transparent)' : 'transparent' }}
        ...
        {activeHover && (
          <div
            className="absolute bottom-2 pointer-events-none transform -translate-x-1/2 bg-surface-container/90 border border-outline-variant text-on-surface text-2xs px-2.5 py-1 rounded-md shadow-2xl backdrop-blur-md z-20 flex items-center gap-1.5"
            style={{
          ...
          >
            ...
            {currentChapter && <span className="text-on-surface-variant truncate max-w-[var(--layout-chart)]">«{currentChapter.title}»</span>}
          </div>
        )}
      </div>

      {chapters.length > 0 && (
        <div className="mt-2 flex gap-1 w-full h-2 rounded overflow-hidden bg-surface-container-lowest/40">
          {chapters.map((ch, i) => {
            const widthPct = Math.max(1, ((ch.endSeconds - ch.startSeconds) / duration) * 100)
            return (
              <div
                key={i}
                className="h-full bg-on-surface/20 hover:bg-secondary transition-colors cursor-pointer relative group"
