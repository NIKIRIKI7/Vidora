...
export const SceneCard = ({ scene, time, description, isActive = false }: SceneCardProps) => (
  <div
    className={`rounded-lg p-3 bg-surface-container/20 border transition-colors group cursor-pointer relative overflow-hidden ${isActive ? 'border-primary/30' : 'border-outline-variant/20 hover:border-primary/30'}`}
  >
    ...
    <div className={`flex justify-between items-center mb-2 ${isActive ? 'pl-2' : ''}`}>
      ...
      <span className="font-mono text-13 text-on-surface-variant">{time}</span>
    </div>
