...
export const SceneSidebar = ({
  ...
  const handleSearchStock = async () => {
    ...
    setIsSearching(true)
    try {
        const res = await fetch(`https://api.pexels.com/videos/search?query=${stockQuery}&per_page=15`, {
            headers: { Authorization: import.meta.env.VITE_PEXELS_API_KEY || '563492ad6f917000010000011cf8d655f013444ca94b0f4438dd8952' }
        })
        const data = await res.json()
        setStockResults(data.videos || [])
    } catch (e) {
        console.error(e)
        onShowNotification('Ошибка поиска футажей', 'error')
    } finally {
  ...
  const handleDownloadStock = async (url: string, filename: string) => {
    ...
    try {
        ...
        const data = await res.json()
        if (data.status === 'ok') {
            onShowNotification(`Футаж скачан в Медиатеку! Вы можете перетащить ${filename} на фрагмент.`, 'success')
        }
    } catch (e) {
        console.error(e)
        onShowNotification('Ошибка скачивания', 'error')
    }
  }
    ...
    <aside className="w-[320px] border-r border-white/10 bg-surface-container/30 flex flex-col shrink-0">
      <div className="p-2 border-b border-white/5 bg-surface-container-lowest/30 flex gap-2">
        <button 
          ...
          className={`flex-1 py-2 rounded-lg text-xs font-medium transition-colors ${tab === 'script' ? 'bg-primary/20 text-primary border border-primary/30' : 'text-on-surface-variant hover:text-white'}`}
        >
          Сценарий
        </button>
        <button 
          onClick={() => setTab('stock')} 
          className={`flex-1 py-2 rounded-lg text-xs font-medium transition-colors ${tab === 'stock' ? 'bg-primary/20 text-primary border border-primary/30' : 'text-on-surface-variant hover:text-white'}`}
        >
          Сток (B-Roll)
        </button>
      </div>
             ...
             <div className="grid grid-cols-2 gap-2 mt-2">
                 {stockResults.map(video => (
                     <div key={video.id} className="relative group rounded-lg overflow-hidden border border-white/10 aspect-[9/16] bg-black">
                         ...
                         <button 
                             onClick={() => handleDownloadStock(video.video_files[0]?.link, `stock_${video.id}.mp4`)}
                             className="absolute bottom-2 right-2 bg-primary text-black p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                         >
