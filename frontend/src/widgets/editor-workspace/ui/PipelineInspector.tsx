...
import { getProjectPath, API } from '../lib/helpers'
        ...
        <section className="flex flex-col gap-3">
          ...
          {activeScene?.fragments.map((frag, i) => (
            <div
              ...
              className="p-3 bg-surface-container-lowest/40 border border-white/5 hover:border-secondary/30 transition-colors rounded-xl flex flex-col gap-2 relative group"
            >
              <Icon
                name="drag_indicator"
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    onClick={() => onRunVoiceGenFragment(activeScene.id, frag.id)}
                    title="Переозвучить только этот фрагмент"
                  >
              ...
              />
              <textarea
                className="text-xs bg-transparent text-on-surface resize-none outline-none custom-scrollbar"
                rows={2}
                value={frag.text}
        ...
        <section className="flex flex-col gap-3">
          ...
          <FieldGroup label="Голосовая модель">
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  const isCustom = project.customVoices?.find(v => v.id === voiceModel)
                  const url = isCustom ? `${API}/api/v1/render/media?path=${encodeURIComponent(isCustom.refAudioPath)}` : `/samples/${voiceModel}.wav`
                  new Audio(url).play().catch(() => onShowNotification('Сэмпл не найден', 'error'))
                }}
                className="p-1.5 bg-white/5 hover:bg-white/10 rounded border border-white/10 text-on-surface-variant hover:text-white"
              >
          ...
          <div className="flex flex-col gap-2 mt-2 border-t border-white/5 pt-3">
            <div className="flex justify-between items-center mb-1">
              ...
              <div className="flex bg-surface-container-lowest border border-white/5 rounded-md p-0.5">
                <button 
                  className={`text-[10px] px-2 py-1 rounded transition-colors ${processScope === 'scene' ? 'bg-primary/20 text-primary' : 'text-on-surface-variant hover:text-white'}`}
                  onClick={() => setProcessScope('scene')}
                >Сцена</button>
                <button 
                  className={`text-[10px] px-2 py-1 rounded transition-colors ${processScope === 'project' ? 'bg-primary/20 text-primary' : 'text-on-surface-variant hover:text-white'}`}
                  onClick={() => setProcessScope('project')}
                >Проект</button>
              </div>
            </div>

            <Button variant="dashed" onClick={() => onProcessAudio('mastering', processScope)} disabled={isGeneratingAudio} className="text-xs border-white/10 hover:border-secondary/50 hover:bg-secondary/10 hover:text-secondary">
              🎙️ Мастеринг (EQ + Normalize)
            </Button>
            <Button variant="dashed" onClick={() => onProcessAudio('silero_vad', processScope)} disabled={isGeneratingAudio} className="text-xs border-white/10 hover:border-accent/50 hover:bg-accent/10 hover:text-accent">
              ✂️ Обрезка пауз (Silero VAD)
            </Button>
          </div>
        </section>
