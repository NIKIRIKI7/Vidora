...
import { Logs, Plus, GripVertical, Minus, Mic, Download, Upload, Trash2, SlidersHorizontal, MicVocal, Play, AlignStartVertical, RotateCcw, Cpu, AudioLines, Code, Copy, Film, FileOutput, FileAudio, Sparkles, Video, Volume2, Unlink } from 'lucide-react'
    ...
    >
      ...
      <div className="flex justify-between items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button className="text-[11px] text-on-surface-variant hover:text-secondary p-1 rounded hover:bg-white/5 transition-colors flex items-center" onClick={() => onOpenBRollModal?.('fragment', frag.id)} title="Добавить B-Roll и адаптировать под разрешение проекта">
            <Video size={13} />
          </button>
      ...
      {frag.bRollFileName && (
        <div className="flex items-center justify-between gap-1.5 px-2 py-1 rounded bg-secondary/10 border border-secondary/20 text-secondary text-[11px]">
          <span className="truncate flex items-center gap-1"><Video size={12} /> {frag.bRollFileName}</span>
          <button onClick={() => onUnlinkFragmentBRoll(frag.id)} className="hover:text-error p-0.5"><Unlink size={12} /></button>
        </div>
      )}
      ...
      <VoiceTagToolbar onInsertTag={insertTag} onToggleCaps={toggleCaps} hasSelection={hasSelection} className="w-full" />
...
export const PipelineInspector = React.memo(({
  ...
  const [activeTab, setActiveTab] = useState<'content' | 'audio' | 'visual' | 'export'>(() => {
    return (localStorage.getItem('vidora:inspector-tab') as 'content' | 'audio' | 'visual' | 'export') || 'content'
  })
