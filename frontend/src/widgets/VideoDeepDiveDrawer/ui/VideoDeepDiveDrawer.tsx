...
export const VideoDeepDiveDrawer = ({ onApplyHookToScenario }: VideoDeepDiveDrawerProps): ReactNode => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-surface-container-lowest/60 backdrop-blur-sm">
      <div className="w-full max-w-2xl h-full bg-surface border-l border-outline-variant shadow-2xl flex flex-col overflow-hidden text-on-surface">
        <div className="p-4 border-b border-outline-variant bg-surface-container-low/60 flex flex-col gap-2.5">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                {candidate.isRocket && (
                  <span className="px-2 py-0.5 bg-gradient-to-r from-warning to-warning text-surface-container-lowest font-extrabold text-xxs rounded-full flex items-center gap-1 uppercase">
              ...
              <h2 className="text-sm font-bold text-on-surface line-clamp-2 leading-snug">{candidate.title}</h2>
            </div>
          ...
          <div className="flex items-center gap-4 text-xs font-mono text-on-surface/80 bg-surface-container-lowest/30 p-2 rounded-lg border border-outline-variant/30">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center h-48 gap-3 text-xs text-on-surface/50">
                ...
                <div className="space-y-4">
                  ...
                  {deepDive?.chapters && deepDive.chapters.length > 0 && (
                    <div className="p-3 bg-surface-container-low border border-outline-variant/30 rounded-xl">
                      <h4 className="text-xs font-bold text-on-surface mb-2">Главы ролика ({deepDive.chapters.length})</h4>
                      <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1 text-xs">
                        {deepDive.chapters.map((ch, idx) => (
                          <div key={idx} className="flex justify-between py-1 border-b border-outline-variant/30 text-on-surface/70">
