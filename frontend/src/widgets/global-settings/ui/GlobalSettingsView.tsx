...
import { Button, Input, Select, FieldGroup, Slider, Spinner, TextArea, SidebarItem, SegmentedControl, PageHeader, ConfirmDialog, EmptyState } from '@shared/ui'
import { Eye, EyeOff, Cloud, Server, Download, RotateCcw, Plus, Trash2, Video, MicVocal, Brain, FileText, Wrench, SlidersHorizontal, Settings } from 'lucide-react'
...
const PromptVersionEditor = ({ label, categoryKey, rows }: { label: string, categoryKey: keyof GlobalPromptSettings, rows: number }) => {
  ...
  return (
    <div className="flex flex-col gap-3 bg-surface-container-lowest/50 p-4 rounded-xl border border-outline-variant/20">
      <div className="flex justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" onClick={addNewVersion} className="p-1.5 text-secondary hover:text-on-surface hover:bg-secondary/20" title="Клонировать и создать новую версию"><Plus size={16} /></Button>
          <Button variant="ghost" disabled={category.versions.length <= 1} onClick={deleteActiveVersion} className="p-1.5 text-error hover:text-on-surface hover:bg-error/20" title="Удалить активную версию"><Trash2 size={16} /></Button>
        </div>
      </div>
      <div className="flex items-center gap-2">
...
export const GlobalSettingsView = ({ onBack, onGoToAudio }: { onBack: () => void; onGoToAudio?: () => void }) => {
  ...
  return (
    <div className="flex flex-col h-dvh w-full bg-background animate-in fade-in duration-300">
      <PageHeader title="Глобальные настройки" icon={Settings} onBack={onBack} />

      <div className="flex-1 flex overflow-hidden max-w-7xl mx-auto w-full">
        <div className="w-64 shrink-0 border-r border-outline-variant/40 p-6 flex flex-col gap-2">
          <SidebarItem icon={Brain} label="AI Движки и API" isActive={activeTab === 'ai'} onClick={() => setActiveTab('ai')} />
          <SidebarItem icon={FileText} label="Промпты LLM" isActive={activeTab === 'prompts'} onClick={() => setActiveTab('prompts')} />
          <SidebarItem icon={Wrench} label="Скиллы (Навыки ИИ)" isActive={activeTab === 'skills'} onClick={() => setActiveTab('skills')} />
          <SidebarItem icon={SlidersHorizontal} label="Аудио и Пайплайн" isActive={activeTab === 'audio'} onClick={() => setActiveTab('audio')} />
          <SidebarItem icon={MicVocal} label="Глобальные голоса" isActive={activeTab === 'voices'} onClick={() => setActiveTab('voices')} />
        </div>
          ...
          <div className="max-w-3xl flex flex-col gap-8 pb-20">
            ...
            {activeTab === 'ai' && (
              <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center gap-3 bg-surface-container-lowest/50 rounded-xl p-4 border border-outline-variant/20">
                ...
                <div className="flex flex-col gap-6 p-6 bg-surface-container/30 border border-outline-variant/20 rounded-2xl">
                  <h3 className="text-lg font-bold text-on-surface mb-2">Глобальные настройки (API Ключи)</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-surface-container-lowest/40 p-5 rounded-xl border border-primary/20 shadow-inner">
                    ...
                    <FieldGroup label={`API Ключ (${cloudProvider === 'routerai' ? 'RouterAI' : 'AITunnel'})`}>
                      <div className="relative">
                        ...
                        />
                        <Button variant="icon" size="sm" icon={showKey ? EyeOff : Eye} onClick={() => setShowKey(!showKey)} className="absolute right-3 top-1/2 -translate-y-1/2" />
                      </div>
                    </FieldGroup>
                  </div>
                  <div className="grid grid-cols-1 gap-6 bg-surface-container-lowest/40 p-5 rounded-xl border border-error/20 shadow-inner mt-4">
                    <FieldGroup label="YouTube Data API v3 Ключ (Для поиска идей)">
                      ...
                      <p className="text-xxs text-on-surface-variant mt-1">Необходим для работы агента-аналитика и поиска вирусных видео.</p>
                    </FieldGroup>
                  </div>
                  <div className="grid grid-cols-1 gap-6 bg-surface-container-lowest/40 p-5 rounded-xl border border-secondary/20 shadow-inner mt-4">
                    <FieldGroup label="Pexels API Key (Для Auto B-Roll Matcher)">
                      ...
                      <p className="text-xxs text-on-surface-variant mt-1">Бесплатный ключ Pexels (api.pexels.com) для поиска и скачивания стоковых B-Roll футажей.</p>
                    </FieldGroup>
                  </div>
                </div>

                <div className="flex flex-col gap-6 p-6 bg-surface-container/30 border border-outline-variant/20 rounded-2xl">
                  <h3 className="text-lg font-bold text-on-surface mb-2">🧠 Сценарии и Идеи (LLM)</h3>
                  <SegmentedControl
                    fill
                    ...
                    options={[
                      ...
                      { value: 'local', label: 'Локально', icon: Server, accent: 'success' },
                    ]}
                  />
                ...
                <div className="flex flex-col gap-6 p-6 bg-surface-container/30 border border-outline-variant/20 rounded-2xl">
                  <h3 className="text-lg font-bold text-on-surface mb-2">🎬 Визуал и Код (Remotion TSX)</h3>
                  <SegmentedControl
                    fill
                    value={taskModes.visual}
                    onChange={(v) => setTaskMode('visual', v)}
                    options={[
                      { value: 'cloud', label: 'Облако', icon: Cloud },
                      { value: 'local', label: 'Локально', icon: Server, accent: 'success' },
                    ]}
                ...
                <div className="flex flex-col gap-6 p-6 bg-surface-container/30 border border-secondary/20 rounded-2xl">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-lg font-bold text-on-surface">🎞️ Подбор B-Roll Футажей (LLM Мозг)</h3>
                  </div>
                  <SegmentedControl
                    fill
                    value={taskModes.broll}
                    onChange={(v) => setTaskMode('broll', v)}
                    options={[
                      { value: 'cloud', label: 'Облако', icon: Cloud },
                      { value: 'local', label: 'Локально', icon: Server, accent: 'success' },
                    ]}
                ...
                <div className="flex flex-col gap-6 p-6 bg-surface-container/30 border border-outline-variant/20 rounded-2xl">
                  <h3 className="text-lg font-bold text-on-surface mb-2">🎙️ Озвучка (TTS)</h3>
                  <SegmentedControl
                    fill
                    value={taskModes.audio}
                    onChange={(v) => setTaskMode('audio', v)}
                    options={[
                      { value: 'cloud', label: 'Облако', icon: Cloud },
                      { value: 'local', label: 'Локально', icon: Server, accent: 'success' },
                    ]}
                ...
                <div className="bg-surface-container-lowest/40 p-5 rounded-xl border border-outline-variant/20 flex flex-col gap-4">
                  <div className="flex items-center gap-2">
                    <Download size={20} className="text-success" />
                    <span className="text-sm font-medium text-on-surface">Скачать модель (Hugging Face / Ollama)</span>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <Button variant="primary" onClick={() => handlePull(hfPullUrl)} disabled={!hfPullUrl || pulling !== null} className="bg-success hover:bg-success/80 text-surface-container-lowest shrink-0 px-6">
            ...
            {activeTab === 'prompts' && (
              <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold text-on-surface">Глобальные Промпты LLM</h3>
                  <Button variant="dashed" onClick={resetGlobalPrompts} className="text-xs text-on-surface-variant hover:text-on-surface">
                    <RotateCcw size={16} className="mr-1" /> Сбросить по умолчанию
                  </Button>
                </div>
                <div className="text-xs text-on-surface-variant bg-surface-container-lowest/50 border border-outline-variant/20 p-4 rounded-xl leading-relaxed font-mono">
            ...
            {activeTab === 'skills' && (
              <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold text-on-surface">Скиллы (Навыки ИИ)</h3>
                </div>
                <p className="text-xs text-on-surface-variant bg-surface-container-lowest/50 border border-outline-variant/20 p-4 rounded-xl leading-relaxed font-mono">
            ...
            {activeTab === 'audio' && (
              <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="bg-surface-container-lowest/50 p-6 rounded-2xl border border-outline-variant/20 flex flex-col gap-6">
                  <h3 className="text-lg font-bold text-on-surface mb-2">Настройки синхронизации (WhisperX)</h3>
                ...
                <div className="bg-surface-container-lowest/50 p-6 rounded-2xl border border-outline-variant/20 flex flex-col gap-6">
                  <h3 className="text-lg font-bold text-on-surface mb-2">Индикаторы удержания зрителя (Pacing)</h3>
            ...
            {activeTab === 'voices' && (
              <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="text-lg font-bold text-on-surface">Глобальные Голоса (Speaker Profiles)</h3>
                ...
                <div className="flex flex-col gap-3">
                  {customVoices.map(gv => (
                    <div key={gv.id} className="p-4 rounded-xl border border-outline-variant/40 bg-surface-container-lowest flex justify-between items-center shadow-md hover:border-primary/30 transition-colors">
                      <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className="flex flex-col gap-0.5 min-w-0">
                          <span className="text-base font-bold text-on-surface flex items-center gap-2">
                            {gv.name}
                            <span className="px-1.5 py-0.5 text-3xs uppercase font-mono rounded bg-on-surface/10 text-on-surface-variant">
                              {gv.source_type}
                            </span>
                          </span>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <Button
                          ...
                          onClick={() => gv.id && setVoiceToDelete({ id: gv.id, name: gv.name ?? '' })}
                        >
                          <Trash2 size={18} />
                        </Button>
                      </div>
                  ...
                  {customVoices.length === 0 && (
                    <EmptyState
                      icon={MicVocal}
                      title="Пока нет кастомных голосов"
                      description="Перейдите в Audio Studio, чтобы клонировать голос или создать новый тембр через ИИ."
                      className="border-2 border-dashed border-outline-variant/40 rounded-2xl"
                    />
                  )}
                </div>
              </div>
      ...
      <ConfirmDialog
        ...
        title="Удалить голос?"
        description={voiceToDelete ? `Безвозвратно удалить голос "${voiceToDelete.name}"?` : undefined}
