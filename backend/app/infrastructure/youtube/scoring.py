...
def cluster_and_rank_signals(raw_signals: List[Dict[str, Any]]) -> List[EarlySignalItem]:
    ...
    for item in raw_signals:
        ...
        if not title:
            continue
        short_key = " ".join(re.sub(r"\s+", " ", title.lower()).split()[:4])
        ...
        cluster["keywords"].update(extract_keywords_from_text(title))

    results: List[EarlySignalItem] = []
    for entry in clusters.values():
        ...
        results.append(EarlySignalItem(
            id=entry["id"], title=entry["title"], query=entry["query"],
            ...
            growth_pct="+5000% (Breakout)" if entry["breakout"] else f"+{int(demand * 7.5)}%",
            metrics={
    ...
    return sorted(results, key=lambda s: s.vps_score, reverse=True)
