...
from app.infrastructure.youtube.normalizer import (
    ...
    parse_count, parse_duration_to_seconds, sanitize_channel_query
)
...
class YtScrapeService:
    ...
    def _get_client(cls, language: str = "ru", region: str = "RU"):
        lang_code, reg_code, _ = normalize_language_code(language)
        key = f"{lang_code}_{reg_code}"
        if key not in cls._clients:
            from ytscrape import YouTube
            cls._clients[key] = YouTube(language=lang_code, region=reg_code)
    ...
    def search_videos_sync(cls, query: str, max_results: int = 60, language: str = "ru") -> List[Dict[str, Any]]:
        ...
        cache_key = f"search_{query}_{max_results}_{lang_code}"
        ...
        for attempt in range(2):
            try:
                ...
                yt = cls._get_client(language=lang_code, region=region)
                time.sleep(0.15)
                ...
                for item in yt.search(query, filter=SearchFilter.VIDEOS, max_results=max_results):
                    ...
                    views = parse_count(
                        getattr(item, 'views', None) or getattr(item, 'view_count', None) or getattr(item,
                                                                                                     'view_count_text',
                                                                                                     0))
                    duration_sec = parse_duration_to_seconds(
                        getattr(item, 'duration', None) or getattr(item, 'length_seconds', None) or getattr(item,
                                                                                                            'duration_seconds',
                                                                                                            0))
                    pub_str = str(getattr(item, 'published_at', '') or getattr(item, 'published', '') or getattr(item,
                                                                                                                 'published_text',
                                                                                                                 '') or '')
                    thumb = getattr(item, 'thumbnail_url', '') or getattr(item, 'thumbnail',
    ...
    def get_channel_info_sync(cls, channel_id_or_handle: str, language: str = "ru") -> Dict[str, Any]:
        ...
        cache_key = f"channel_{clean_target}_{lang_code}"
        cached = cls._get_cached(cache_key)
        if cached is not None:
            return cached
        ...
        for attempt in range(2):
            try:
                ...
                if ch:
                    subs_raw = getattr(ch, 'subscriber_count', None) or getattr(ch, 'subscribers', None) or getattr(ch,
                    ...
                    if ch_vids:
                        for v in list(ch_vids)[:25]:
                            ...
                            if vid_id:
                                dur = parse_duration_to_seconds(
                                    getattr(v, 'duration', 0) or getattr(v, 'length_seconds', 0))
                                res["videos"].append({
                                    "video_id": vid_id,
                                    ...
                                    "is_short": 0 < dur <= 180,
                                    "published_at": str(
                                        getattr(v, 'published_at', '') or getattr(v, 'published', '') or getattr(v,
                                                                                                                 'published_text',
                                                                                                                 '')),
    ...
    def get_comments_sync(cls, video_id: str, max_comments: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for c in raw_comments:
                ...
                if text.strip():
                    comments.append(
                        {"text": text.strip(), "author": getattr(c, 'author', '') or 'User', "likes": likes})
            ...
            if t_obj:
                if isinstance(t_obj, str):
                    ...
                    for segment in t_obj:
                        seg_text = getattr(segment, 'text', '') if not isinstance(segment, dict) else segment.get(
                            'text', '')
                        if seg_text:
