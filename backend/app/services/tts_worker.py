...
def get_qwen_model(model_id: str, device: str):
    ...
    print(f"[TTS Worker] Загрузка Qwen3TTS: {model_id} на {device}...", file=sys.stderr, flush=True)
...
def get_moss_model(model_id: str, codec_path: str, device: str):
    ...
    on_cuda = device.startswith("cuda")
    print(f"[TTS Worker] Загрузка MOSS-TTS: {model_id} на {device}...", file=sys.stderr, flush=True)
    ...
    processor = AutoProcessor.from_pretrained(
        model_id, trust_remote_code=True, codec_path=codec_path
    )
    ...
    if engine == "qwen":
        ...
        if mode == "design":
            wavs, sr = model.generate_voice_design(
                text=text, language=language, instruct=design_prompt
            )
        elif mode == "clone":
            ...
            wavs, sr = model.generate_custom_voice(
                text=text, language=language,
                speaker=speaker, instruct=design_prompt or None,
            )
