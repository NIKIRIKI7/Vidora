...
class AgentReq(BaseModel):
    ...
    api_keys: dict = {}  # Ключи для LLM (routerai/aitunnel)
...
# --- РОУТ АГЕНТА ИДЕЙ (SSE Стриминг) ---
@router.post("/agent/stream")
async def stream_agent_ideas(req: AgentReq):
    try:
        agent = YouTubeIdeaAgent(
            llm_engine=req.llm_engine,
            api_key=req.youtube_key,
...
# --- ГЕНЕРАЦИЯ ПРЕВЬЮ: ThumbnailPromptEngine + правила DESIGN.md ---
@router.post("/thumbnail-prompt")
async def generate_thumb_prompt(req: PromptReq):
    try:
        concept_json = await thumb_engine.generate_concept(
            video_title=req.video_title,
            transcript=req.transcript,
            engine=req.engine,
            api_keys=req.api_keys
        )
        return {"status": "ok", "concept": concept_json}
    except Exception as e:
