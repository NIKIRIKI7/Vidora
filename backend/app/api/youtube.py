...
async def stream_agent_ideas(req: AgentReq):
    try:
        agent = YouTubeIdeaAgent(llm_engine=req.llm_engine, api_key=req.youtube_key or "", api_keys=req.api_keys)
        return StreamingResponse(
            agent.run_pipeline(req.query, req.settings, req.project_path),
...
async def draft_script(req: DraftReq):
    try:
        agent = YouTubeIdeaAgent(llm_engine=req.engine, api_keys=req.api_keys)
        res = await agent.draft_script(
            req.title, req.idea_description, req.channel_context,
            video_type=req.video_type, target_duration=req.target_duration,
            custom_prompt=req.custom_prompt, audio_engine=req.audio_engine,
            audience_comments=req.audience_comments
        )
...
async def get_comments(req: CommentsReq):
    try:
        comments = await yt_manager.get_video_comments(req.video_id, req.max_comments)
...
async def analyze_channel(req: AnalyzeChannelReq):
    try:
        agent = YouTubeIdeaAgent(llm_engine=req.engine, api_key=req.youtube_key or "", api_keys=req.api_keys)
...
async def download_meta(req: dict):
    try:
        out_dir = f"{req.get('project_path', 'vidora_projects')}/assets/refs"
